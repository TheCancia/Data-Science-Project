...
## Idea di base 
Tutti i Trader vorrebbero essere capaci di predirre il futuro, sapere quale titolo sarà il nuovo BTC oppure il nuovo TSLA
oppure quando ci sarà il prossimo sellOff oppure addirittura il prossimo Black Monday.

Pensare di poter predirre questi eventi sembra quasi fantascienza, e in buona parte lo è, in questi anni di Trading ho 
capito che nulla è deciso e non ci sarà mai un modello matematico che predirrà perfettamente questi futuri avvenimenti. 
Però esiste un filosofia di Trading chiamata Quantitative Trading (che personalmente seguo), che si prefiggie l'idea di 
analizzare il prezzo e il volume dei titoli tramite modelli matematici. 
Personalmente mi trovo sulla punta dell'Iceber per quanto riguarda questi Modelli, ma tramite questa presentazione il mio 
intento è quello di presentare due Modelli che provano a predirre il Rendimento futuro dei Titoli Azionari. 

# Quali sono questi Modelli ? 
 La spina dorsale di questa analisi si basa su 2 modelli e una teoria : 

   1) Il Secondo pilastro della Teoria di John Murphy, ovvero : " Il Mercato si Ripete" 
...
Andiamo ad applicare questi modelli ad R. 
...
## Otteniamo qualche Dato 

Vista la natura volatile del mercato azionario mi sono prefissato l'idea di non usare un dataset pre-definito, visto che
volendo possiamo effettuare questa analisi con intervalli di tempo nell'ordine dei secondi. 
Per sopperire alla mancanza di un dataset introduciamo la funzione getSymbols che, tramite procedure di Web Scraping, ottiene
da Yahoo Finance i dati di un titolo. 
...
TSLA_log_returns %>%
  ...
  geom_rug(alpha = 0.5)
```
Purtroppo non abbiamo una chiara corrispondenza a livello grafico con la distribuzione normale, questo probabilmente è dovuto ad i movimenti
molto incerti del mercato azionario attuale. 
...
dist_log_returns
```
abbiamo una suddivisione netta tra dati positivi e negativi sottolineata tra i quantili del 50% al 75% 
Potremo introdurre un'ulteriore valore (60%) per capire come si distribuisce il rendimento. 
...
Questa è solamente UNA delle possibili proiezioni relative al titolo TSLA, il passo successivo è eseguire N iterazioni della Random Walk.

## Come definiamo la Simulazione di Monte Carlo ? 
La simulazione di MonteCarlo è una tecnica di risoluzione dei problemiu usati per approssimare la probabilità di certi risultati eseguendo N prove, definite come simulazioni. 
Tramite questa simulazione abbiamo la possibilità di renderci conto della casualità di un parametro di Trading che tipicamente viene 
attribuito alla sequenza di operazioni. 
La simulazione di monte carlo ci permette di eseguire in quantità ripetute la simulazione di Monte Carlo, andremo ad eseguire la Random Walk per 250 iterazioni. 
```{r}
# definiamo i parametri 
...
Il Tasso Composto di Crescita Annuo o CAGR (per usare acronimi anglosassoni), rappresenta la crescita percentuale media di una grandezza in 
un lasso di tempo; solitamente questo tasso viene usato per determinare il fatturato di una Azione all'anno, fornendoci la crescita 
percentuale.
...
diff
```
In Conclusione io mi ritengo soddisfatto, abbiamo ottenuto una differenza di $0.01476$ tra il CAGR e la simulazione, ci siamo avvicinati ai rendimenti in media teorici. 

Ovviamente bisogna ricordare che la Legge di J.J.Murphy non è propiamente una legge, non abbiamo la sicurezza che il passato 
si ripeta, il vantaggio indubbio del CAGR è nel fatto che dato il valore iniziale e quello finale si ottenie il rendimento medio annuo e quindi una indicazione temporale utile. 
